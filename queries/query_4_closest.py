...
from pyspark.sql.functions import col, regexp_replace, desc, year, to_date, unix_timestamp, when, udf, count, avg, format_number, row_number, monotonically_increasing_id
...
crime_x_stations_df = crime_incidents_df.join(LAPD_Police_Stations_df) \
        .withColumn("year", year("DATE OCC")) \
        .withColumn("distance", get_distance_udf(col("LAT"), col("LON"), col("Y"), col("X")))


closest_window = Window.partitionBy("ID").orderBy("distance")

crime_x_station_df = crime_x_stations_df \
        .withColumn("number_in_partition", row_number().over(closest_window)) \
        .filter(col("number_in_partition") == 1) \
        .persist()
